...
## Rotation components: element-tree stability, status bar passthroughs, freezeWhileLocked

- **`FakeLandscapeView` renders the same element tree at every angle** (outer `View` > inner `View` > children); only style values differ. It used to return a different tree per angle, which made React unmount/remount all children on every 0 <-> ±90 rotation (wiping their state; ~700-800ms JS per rotation in Solitaire). Never branch the returned tree on rotation again; `FakeLandscapeView.test.tsx` has regression tests counting mounts across 0/90/-90/180 for several style shapes. At 0°/180° the caller's flattened `style` is split (`OUTER_STYLE_KEYS` in the component): the outer view gets everything that positions/sizes it in its parent (position/insets, zIndex, flex*, width/height/min/max, aspectRatio, alignSelf, margins, display, transform), the inner container gets the rest plus `flexGrow:1/flexShrink:1/flexBasis:auto` and the 180° rotate, so layout matches the original single-`<View style>` (absolute overlays stay out of flow; alignItems/justifyContent apply to the inner). An earlier iteration used a plain `flex: 1` outer with the whole style on the inner, which broke overlay/`alignSelf`/explicit-size callers (LightCycles/Snake corner chips) - don't reintroduce it. ±90° is unchanged from the original (outer absoluteFill box-none, inner = style + swapped size/position + rotate). The jest `react-native` mock's `StyleSheet` has a real `flatten` and `absoluteFill` for these tests.
- **`RotationAwareStatusBar`** gained optional `style`/`animated`/`hideTransitionAnimation` passthroughs to expo-status-bar (undefined = expo's defaults). `hidden` stays `rotation !== 0`.
- **`OrientationProvider`'s opt-in `freezeWhileLocked`** (default false) freezes the shared reading itself while `useOrientationLock().locked` is true, implemented in `useOrientationStateSource(deviceMotion, hold)`: the sensor listener discards the pending candidate and commits nothing while held (via a ref, so no resubscribe), which also means unlock restarts the COMMIT_MS debounce from scratch. Hold only applies once `state.resolved`, so a launch-time lock latches the first confident reading instead of pinning DEFAULT_STATE. `OrientationStateProvider` reads the lock via `useOrientationLockSettings()` (inert default when no lock provider). Native only; web's reading is window-derived and unaffected. Per-call-site `locked` args are unchanged and still work. Tests: `OrientationProvider.freezeWhileLocked.test.tsx`.
